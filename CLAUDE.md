...
| 049 | **Fix RLS `partidos_rapidos_update`:** la policy solo permitía tocar filas donde `auth.uid()` ya fuera `capitan_a_id`/`capitan_b_id` — bloqueaba silenciosamente `join_partido_id` (unirse a un trío ajeno es exactamente el caso en que el usuario todavía no es ninguno de los dos). Se agrega `OR (estado = 'buscando' AND capitan_b_id IS NULL)` | — |

> ✅ Migraciones 001–048 aplicadas en producción (verificado 2026-09-24 con `supabase migration list`); la 049 corrige un bug encontrado en integration tests — pendiente de `db push`.
...
**Tests de integración** (`tests/integration/partidos-rapidos-{reto,matching,resultado}.test.ts`) — siguen el patrón de `aceptar-original.test.ts`/`ranking-1v1-atomic.test.ts`: reto directo (pendiente→aceptar/rechazar/cancelar, con y sin compañeros, `es_vs_king` al retar al Rey), matching ciego + join explícito + concurrencia real de `emparejar_partido_rapido()` (RPC directa, no vía ruta — el harness comparte un único cliente Supabase "activo" y no puede simular dos requests concurrentes de usuarios distintos), y resultado (XP con trío desparejo, Rey 3v3 por capitán, regresión de la disputa que dejaba el partido sin forma de volver a proponer). Requieren Supabase local (`supabase start` + `.env.test`, ver `.env.test.example`). Corridos localmente el 2026-09-24 contra Docker Desktop en Windows — encontraron 2 bugs reales (no del harness): la policy RLS `partidos_rapidos_update` bloqueaba silenciosamente `join_partido_id` (fix: migración 049) y el PATCH de `resultado` devolvía 409 en vez del `yaConfirmado` idempotente porque gateaba en `partido.estado !== 'resultado_pendiente'` antes de chequear si el resultado ya estaba confirmado (el propio `confirmar` deja el partido en `'completado'`). Ambos arreglados; los 30 tests de la suite completa pasan en verde tras un `db reset`.
...
- **Partido Rápido 3v3**: migraciones 047+048+**049** (pendiente `db push` a producción, ver más abajo); probado con `tsc`/build + smoke test de rutas; tests de integración (`tests/integration/partidos-rapidos-*.test.ts`) corridos localmente el 2026-09-24 (Docker Desktop + `supabase start`) — 2 bugs reales encontrados y arreglados (ver sección "Partido Rápido 3v3"), suite completa en verde (30/30) tras el fix. No probado end-to-end con dos usuarios reales en la app corriendo.
- **Migración 049 pendiente de aplicar** — `supabase db push` (fix RLS de `join_partido_id`, ver sección "Partido Rápido 3v3" y tabla de migraciones). Sin este push, "unirse a un trío que ya está buscando" sigue devolviendo 409 falso en producción incluso en el primer intento.
