...
export default async function InvitacionesPage() {
  ...
  return (
    <div className="p-5 max-w-2xl">
      <h1 className="text-[18px] font-semibold text-on-surface mb-1">Invitar jugadores</h1>
      <p className="text-[13px] text-on-surface-variant mb-6">
        Suma jugadores a <span className="text-on-surface font-medium">{equipo.nombre}</span> por email, WhatsApp o link copiable.
      </p>

      <InvitacionForm equipoId={equipo.id} linkToken={linkToken} />

      <div className="mt-6">
        <div className="text-[10px] text-outline tracking-[0.08em] mb-3 font-medium uppercase">
        ...
        {!pendientes?.length ? (
          <div className="bg-surface-container-low border border-outline-variant rounded-xl p-6 text-center">
            <div className="text-[13px] text-on-surface-variant">Nadie esperando respuesta</div>
            <div className="text-[11px] text-outline mt-1">Las invitaciones que envíes aparecen acá hasta que las acepten.</div>
          </div>
        ) : (
          <div className="bg-surface-container-low border border-outline-variant rounded-xl overflow-hidden">
            {pendientes.map((inv, i) => {
              ...
              return (
                <div
                  key={inv.id}
                  className={`flex items-center gap-2.5 px-4 py-3 ${i < pendientes.length - 1 ? 'border-b border-outline-variant' : ''}`}
                >
                  <div className="flex-1 min-w-0">
                    <div className="text-[12px] text-on-surface truncate">{contacto}</div>
