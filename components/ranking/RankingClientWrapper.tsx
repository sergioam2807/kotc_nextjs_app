    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 pt-4 flex gap-1 flex-shrink-0">
        <button
          ...
          className={`px-4 py-1.5 rounded-lg text-[12px] font-semibold transition-colors ${tab === 'equipos' ? 'bg-accent/15 text-accent' : 'text-outline hover:text-on-surface-variant'}`}
        >
          Equipos
        </button>
        <button
          onClick={() => setTab('jugadores')}
          className={`px-4 py-1.5 rounded-lg text-[12px] font-semibold transition-colors ${tab === 'jugadores' ? 'bg-accent/15 text-accent' : 'text-outline hover:text-on-surface-variant'}`}
        >
          Jugadores
        </button>
      </div>
