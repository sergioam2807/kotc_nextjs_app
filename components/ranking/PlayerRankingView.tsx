    ...
    <>
          ...
          <div className="flex items-end justify-center gap-2 sm:gap-3">
            ...
            {stats[1] && (
              <Link href={`/jugadores/${stats[1].id}`} className="flex flex-col items-center gap-2 flex-1 hover:opacity-80 transition-opacity">
                ...
                <div className="text-[10px]" style={{ color: MEDAL.silver.text }}>{stats[1].xp.toLocaleString('es-CL')} XP</div>
                <div
            ...
            <Link href={`/jugadores/${stats[0].id}`} className="flex flex-col items-center gap-2 flex-1 hover:opacity-80 transition-opacity">
              ...
              <div className="text-[11px] font-semibold" style={{ color: MEDAL.gold.text }}>{stats[0].xp.toLocaleString('es-CL')} XP</div>
              <div
            ...
            {stats[2] && (
              <Link href={`/jugadores/${stats[2].id}`} className="flex flex-col items-center gap-2 flex-1 hover:opacity-80 transition-opacity">
                ...
                <div className="text-[10px]" style={{ color: MEDAL.bronze.text }}>{stats[2].xp.toLocaleString('es-CL')} XP</div>
                <div
      ...
      <div className="px-4 pb-6">
        {stats.map((player, idx) => {
          // Porcentaje real de progreso dentro del nivel actual
          const pct = porcentajeEnNivel(player.xp, player.nivel);
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  ...
                  {player.id === currentUserId && (
                    <span className="text-[9px] text-accent bg-accent/15 px-1 rounded-sm flex-shrink-0">tú</span>
                  )}
                </div>
                {player.equipoNombre && (
                ...
                {/* XP bar — progreso real dentro del nivel actual */}
                <div className="w-full h-0.5 bg-outline-variant rounded-full mt-1.5">
                  <div
                ...
                <div className="text-right min-w-[44px]">
                  ...
                  >
                    <span className={idx > 2 ? 'text-on-surface-variant' : undefined}>
                      {player.xp.toLocaleString('es-CL')}
                    </span>
                  </div>
